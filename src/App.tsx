...
const survivedBackgrounds = [
  "/images/survived1.webp",
  "/images/survived2.webp",
];
const diedBackgrounds = ["/images/died1.webp", "/images/died2.webp"];

// ------------------ Generate Deck ------------------
const initialDeckCount = 44;
const ranks = [
  "2",
  "3",
  "4",
...
export default function App() {
  ...
  const [lastPotionValue, setLastPotionValue] = useState(null);
  ...
  useEffect(() => {
    if (turnActionCount === 0) {
      const played = initialDeckCount - (deck.length + room.length);
      const roomNumber = Math.floor(played / 4) + 1;
      if (gameOver && currentHP > 0) {
        const randomIndex = Math.floor(
          Math.random() * survivedBackgrounds.length
        );
        setRoomBackground(survivedBackgrounds[randomIndex]);
      } else if (roomNumber === 11) {
        setRoomBackground(lastRoomBackground);
      } else {
        const randomIndex = Math.floor(Math.random() * roomBackgrounds.length);
        setRoomBackground(roomBackgrounds[randomIndex]);
      }
    }
  }, [deck, room, gameOver, currentHP, turnActionCount]);
  ...
  useEffect(() => {
    if (gameOver && currentHP <= 0) {
      const randomIndex = Math.floor(Math.random() * diedBackgrounds.length);
      setRoomBackground(diedBackgrounds[randomIndex]);
    }
  }, [gameOver, currentHP]);
  ...
  function processMonsterFight(choice, card, index) {
    ...
    if (choice === "barehanded") {
      newHP -= card.value;
      msg = `Barehanded fight with ${getMonsterName(card)} (Power: ${
        card.value
      }). Took ${card.value} damage.`;
    } else if (choice === "weapon") {
      if (equippedWeapon) {
        const { value: wVal, lastMonsterKilledValue } = equippedWeapon;
        const canUse =
          lastMonsterKilledValue === null ||
          card.value <= lastMonsterKilledValue;
        if (canUse) {
          ...
          msg = `Fought ${getMonsterName(card)} (Power: ${
            card.value
          }) with weapon. Took ${dmg} damage.`;
        } else {
          newHP -= card.value;
          msg = `${getMonsterName(card)} (Power: ${
            card.value
          }) cannot be fought with the weapon. Barehanded instead. Took ${
            card.value
          } damage.`;
        }
      } else {
        newHP -= card.value;
        msg = `No weapon equipped. Barehanded fight with ${getMonsterName(
          card
        )} (Power: ${card.value}). Took ${card.value} damage.`;
      }
    }
    const newRoom = [...room];
  ...
  // Deck display calculations
  const maxVisible = 4;
  const cardWidth = 160;
  const overlap = 10;
  const visibleCount = Math.min(deck.length, maxVisible);
  const displayDeck =
    deck.length > maxVisible ? deck.slice(deck.length - maxVisible) : deck;
  const extraCount = deck.length > maxVisible ? deck.length - maxVisible : 0;
              ...
              <div className="relative w-40 h-40 sm:w-80 sm:h-80 flex justify-center">
                {displayDeck.map((card, index) => (
                  <div
                    key={index}
                    className="absolute"
                    style={{ left: `${index * overlap}px`, zIndex: index }}
                  >
                    <img
